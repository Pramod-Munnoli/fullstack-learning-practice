    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                showFlash('success', "Welcome to Wanderlust, " + response.data.user.username + "!");
                navigate('/');
            }
        } catch (err) {
            showFlash('error', err.response?.data?.message || "Signup failed");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="flex justify-center items-center py-20 px-8 min-h-[80vh] relative group overflow-hidden">
            ...
            <div className="bg-white/80 backdrop-blur-xl p-12 rounded-[50px] shadow-2xl border border-white/50 w-full max-w-md space-y-10 relative z-10 transform hover:scale-[1.01] transition-transform duration-500">
                <div className="text-center space-y-3">
                    <h1 className="text-5xl font-black text-gray-800 tracking-tighter">Join the Adventure</h1>
                    <p className="text-gray-400 font-medium tracking-wide italic">Start your journey today!</p>
                </div>
                    ...
                    >
                        {loading ? (
                            ...
                            <span className="flex items-center justify-center gap-2 group-hover:tracking-widest transition-all">
                                JOIN NOW <span className="opacity-50 group-hover:opacity-100 translate-x-2">&rarr;</span>
                            </span>
                        )}
                    </button>
                </form>
                ...
                <div className="text-center">
                    <p className="text-gray-400 font-medium">
                        Already with us? <Link to="/login" className="text-red-500 hover:text-red-600 font-black italic underline decoration-red-100 hover:decoration-red-500 underline-offset-8 transition-all">Login Instead</Link>
                    </p>
                </div>
            </div>
